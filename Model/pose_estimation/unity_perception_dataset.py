...
def _load_config() -> EasyDict:
    with open(_config_path, "r") as stream:
        return EasyDict(yaml.load(stream, Loader=yaml.FullLoader))


def _get_rescale_transform(scale: float) -> Compose:
    return Resize((scale, scale))
    # return Compose([Resize((scale, scale)), ToTensor()])
...
class UnityPerceptionDataset(torch.utils.data.Dataset):
    ...
    def __init__(self, config: EasyDict) -> None:
        ...
        self._data_root: Final[str] = config.dataset.root
        logging.info("Loading dataset from root path: %s", config.dataset.root)
        captures = Captures(
            data_root=self._data_root, version=config.dataset.schema_version
        )
        self._data: Final[DataFrame] = captures.filter(
            def_id=config.dataset.annotation_id
        )
    ...
    def __getitem__(
        ...
        """Return the image and label for the specified index.
        ...
        transform = _get_rescale_transform(scale)
...
if __name__ == "__main__":
    ...
    dataset = UnityPerceptionDataset(config)
    print(dataset[234])

# class FastDataset(torch.utils.data.Dataset):
#     """Use a map to files based on non-negative integers.
